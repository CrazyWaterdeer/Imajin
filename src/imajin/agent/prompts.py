...
- Choosing a statistical test: `compare_groups(test="auto")` already picks parametric vs
  ...
  *family*, but YOU must assert the DESIGN. For a paired / within-subject comparison (the same
  ...
  paired. For 3+ groups `compare_groups` returns the omnibus p AND a multiplicity-corrected
  post-hoc under `posthoc` (Games-Howell for ANOVA, Dunn's + Holm for Kruskal) — read that to say
